...
if (AGAINST_FREQ_SECTION == 1)
    FREQUENCY = [0.5, 1, 5, 10, 20, 50, 100, 500];
else
...
fn_handles = {@(x) window_fn_Prodro(x,1,-2)};
...
%     D = zeros(length(FREQUENCY),length(tspan(iii,:)));
    ...
    ode = @(t,D) window_dD_dt(t,D,delta_D, D_min,kappa, input_fn , window_fn{1} );
...
windw_func = func2legend(fn_handles);
...
hold all
for ii = 1:length(FREQUENCY)
    plot(((1:length(tspan(ii,:)))./length(tspan(ii,:))) , y(ii,:),get_line_spec( ii ), 'LineWidth',2.0)
end
ylabel('State Variable y')
xlabel('Normalised Time - t/\omega (s)')
leg_handle=legend(str_leg);
set(leg_handle,'location','NorthEast')
set(leg_handle,'FontSize',10);
line(linspace(0,1,length(tspan(1,:))),ones(1,length(tspan)),'Color', 'k', 'LineStyle','--')
title(windw_func{1});
axis tight
grid
