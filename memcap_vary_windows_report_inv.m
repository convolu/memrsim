...
for ii = 1:length(fn_handles)
    plot(input_v(1,1:end-1), I(ii,:) , get_line_spec( ii ), 'LineWidth',1.5)
end
ylabel('Current - I (A)')
...
hold all
for ii = 1:length(fn_handles)
    plot(tspan, y(ii,:),get_line_spec( ii ),'LineWidth',1.5)
end
...
set(leg_handle,'FontSize',9);
line(tspan,ones(1,length(tspan)),'Color', 'k', 'LineStyle','--', 'LineWidth',2.0)
axis tight
...
hold all
for ii = 1:length(fn_handles)
    plot(tspan, C(ii,:), get_line_spec(ii), 'LineWidth', 2.0);
end
ylabel('Memcapacitance')
xlabel('Time - t (s)')
line(tspan,D_min*ones(1,length(tspan)),'LineWidth', 2.0,'LineStyle','-.');
line(tspan,D_max*ones(1,length(tspan)),'LineWidth', 2.0,'LineStyle','-.');
leg_handle = legend(str_leg);
set(leg_handle,'location','SouthEast')
